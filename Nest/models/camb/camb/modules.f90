    ...
    !FYCR: parameters for the neutrino opacity
    real(dl)  :: G_nu_eff, T_nu_0, interacting_fraction, Num_Nu_interacting
    ...
    real(dl) grhom,grhog,grhor,grhob,grhoc,grhov,grhornomass,grhok,grhor_int
    ...
    real(dl) Nnow,N_nu_now,T_nu_dec_approx,a_nu_dec_approx,tau_nu_dec
    ...
    logical WantReion
    integer nu_i,actual_massless,actual_interacting
    external GetOmegak
    ...
    if (CP%Num_nu_massive > 0) then
        if (.not. CP%Nu_mass_splittings) then
            !Default totally degenerate massesinteracting_fraction
    ...
    if (CP%same_neutrino_Neff) then
        ...
        if (actual_massless + CP%Num_Nu_massive /= 0) then
            ...
            grhornomass=grhor*actual_massless*(1.0d0-CP%interacting_fraction)
        else
            grhornomass=grhor*CP%Num_Nu_massless*(1.0d0-CP%interacting_fraction)
        end if
    else
        grhornomass=grhor*CP%Num_Nu_massless*(1.0d0-CP%interacting_fraction)
    end if

    !Energy density in interacting Neutrinos
    grhor_int =  7._dl/8*(4._dl/11)**(4._dl/3)*grhog*CP%Num_Nu_massless*CP%interacting_fraction

    !Number density of interacting neutrinos today
    CP%Num_Nu_interacting = (CP%Num_Nu_massless + CP%Num_Nu_massive)*CP%interacting_fraction
    actual_interacting = int(CP%Num_Nu_interacting + 1e-6)

    if (actual_interacting /= 0) then
        CP%T_nu_0 = (1.0d0+(CP%Num_Nu_interacting-actual_interacting)/actual_interacting)**(1.0d0/4.0d0)* &
                    (4.0d0/11.0d0)**(1.d0/3.0d0)*CP%tcmb

        N_nu_now = actual_interacting*2.0d0*(3.0d0/4.0d0)*(1.2020569031595942854_dl/const_pi**2)* &
                   CP%T_nu_0**(3.0d0)*(2*const_pi*k_B/(h_P*c))**3
    else !then the interacting neutrinos are cooler than the free-streaming ones
        actual_interacting = ceiling(CP%Num_Nu_interacting +1e-6)
        CP%T_nu_0 = (1.0d0+(CP%Num_Nu_interacting-actual_interacting)/actual_interacting)**(1.0d0/4.0d0)* &
                    (4.0d0/11.0d0)**(1.d0/3.0d0)*CP%tcmb
    ...
    adotrad = sqrt((grhog+grhornomass+grhor_int+sum(grhormass(1:CP%Nu_mass_eigenstates)))/3)
...
500     close(fileio_unit)
    ...
    Transfer_tot=7, Transfer_r_int=8

    integer, parameter :: Transfer_max = Transfer_r_int
        ...
        if (FileNames(i) /= '') then
            ...
            do ik=1,MTrans%num_q_trans
                if (MTrans%TransferData(Transfer_kh,ik,i)/=0) then
                    write(fileio_unit,'(8E14.6)') MTrans%TransferData(Transfer_kh:Transfer_max,ik,i)
                end if
            end do
            close(fileio_unit)
        end if
    ...
    integer,parameter :: nthermo_nu=40000
    ...
    real(dl), dimension(:), allocatable :: dopac_nu, opac_nu
    logical, parameter :: dowinlens = .false.

    !FYCR Parameters for neutrino decoupling
    real(dl) dotmu_nu(nthermo_nu), ddotmu_nu(nthermo_nu)
    real(dl) dddotmu_nu(nthermo_nu),ddddotmu_nu(nthermo_nu)
    real(dl) sdotmu_nu(nthermo_nu),emmu_nu(nthermo_nu)
    real(dl) demmu_nu(nthermo_nu)
    real(dl) tauminn_nu,dlntau_nu,Maxtau_nu
    real(dl) tight_tau_nu
    real(dl) dec_tau_nu
    ...
    public thermo,thermo_nu,inithermo,vis,opac,expmmu,dvis,dopac,ddvis,lenswin, tight_tau,&
    Thermo_OpacityToTime,matter_verydom_tau, ThermoData_Free,&
    z_star, z_drag,  tight_tau_nu,opac_nu,dopac_nu, &
    Thermo_OpacityToTime_nu   !!JH for updated BAO likelihood.

    contains

    function Thermo_OpacityToTime_nu(opacity_nu)
         real(dl), intent(in) :: opacity_nu
         integer j
         real(dl) Thermo_OpacityToTime_nu
         !Do this the bad slow way for now..
          !The answer is approximate
         j =1
         do while(dotmu_nu(j)> opacity_nu)
            j=j+1
         end do

         Thermo_OpacityToTime_nu = exp((j-1)*dlntau_nu)*tauminn_nu

    end function Thermo_OpacityToTime_nu

    subroutine thermo_nu(tau,opacity_nu, dopacity_nu)  !FYCR
        !Compute neutrino opacity
        !If requested also get time derivative of opacity
        implicit none
        real(dl) tau,opacity_nu
        real(dl), intent(out), optional :: dopacity_nu
        integer i
        real(dl) d

        d=log(tau/tauminn_nu)/dlntau_nu+1._dl
        i=int(d)
        d=d-i
        if (i < 1) then
            !Linear interpolation if out of bounds (should not occur).
            opacity_nu=dotmu_nu(1)+(d-1)*ddotmu_nu(1)
            stop 'neutrino thermo out of bounds'
        else if (i >= nthermo_nu) then
            opacity_nu=dotmu_nu(nthermo_nu)+(d-nthermo_nu)*ddotmu_nu(nthermo_nu)
            if (present(dopacity_nu)) then
                dopacity_nu = 0
                stop 'neutrino thermo: shouldn''t happen'
            end if
        else
            !Cubic spline interpolation.
            opacity_nu=dotmu_nu(i)+d*(ddotmu_nu(i)+d*(3*(dotmu_nu(i+1)-dotmu_nu(i)) &
                -2*ddotmu_nu(i)-ddotmu_nu(i+1)+d*(ddotmu_nu(i)+ddotmu_nu(i+1) &
                +2*(dotmu_nu(i)-dotmu_nu(i+1)))))

            if (present(dopacity_nu)) then
                dopacity_nu=(ddotmu_nu(i)+d*(dddotmu_nu(i)+d*(3*(ddotmu_nu(i+1)  &
                    -ddotmu_nu(i))-2*dddotmu_nu(i)-dddotmu_nu(i+1)+d*(dddotmu_nu(i) &
                    +dddotmu_nu(i+1)+2*(ddotmu_nu(i)-ddotmu_nu(i+1))))))/(tau*dlntau_nu)

            end if
        end if

    end subroutine thermo_nu

    subroutine inithermo_nu(taumin,taumax)  !!!FYCR
        use constants
        use precision
        use ModelParams
        ...
        real(dl) adot
        integer i,j1
        real(dl) spline_data_nu(nthermo_nu)
        real(dl) vis_nu !,trans_vis,denom,trans_vis0,denom0,F_nu_int
        ...
        tight_tau_nu = 0
        dec_tau_nu = 0
        tauminn_nu=0.05d0*taumin
        dlntau_nu=log(CP%tau0/tauminn_nu)/(nthermo_nu-1)
        !print*, tauminn_nu, taumax
!  Initial conditions: assume radiation-dominated universe.
        tau01=tauminn_nu
        adot0=adotrad
        a0=adotrad*tauminn_nu
        a02=a0*a0

!  Assume that any entropy generation occurs before tauminn.
!  This gives wrong temperature before pair annihilation, but
!  the error is harmless.
        !Note that G_nu_eff is in units of 1/MeV**2, T_nu_0 is in Kelvin
        dotmu_nu(1) = N_nu_now*Mpc*(CP%G_nu_eff*(1.0d-6*h_P*c/(2*const_pi*eV))**2)**2* &
                    (2*const_pi*k_B*CP%T_nu_0/(h_P*c))**2/(a02*a02)
        sdotmu(1)=0
        !open(unit=25,file='opacity_neutrinos_primode.dat',status='replace')
        do i=2,nthermo_nu
            tau=tauminn_nu*exp((i-1)*dlntau_nu)
            dtau=tau-tau01
!  Integrate Friedmann equation using inverse trapezoidal rule.
            ...
            adot=1/dtauda(a)
            a2 = a*a
! Calculation of the neutrino opacity function
             dotmu_nu(i) = N_nu_now*Mpc*(CP%G_nu_eff*(1.0d-6*h_P*c/(2*const_pi*eV))**2)**2* &
                    (2*const_pi*k_B*CP%T_nu_0/(h_P*c))**2/(a2*a2)

             !Tight coupling switch time when k/opacity is smaller than 1/(tau*opacity)
             if (tight_tau_nu==0 .and. 1/(tau*dotmu_nu(i)) > 0.005) then
                tight_tau_nu = tau !0.005
                print*, 'tight_tau_nu = ', tight_tau_nu
             end if
            

             if (dec_tau_nu==0 .and. 1/(tau*dotmu_nu(i)) > 0.99) then
                dec_tau_nu = tau
                z_dec_nu = 1/a -1.0
                print*, 'dec_tau_nu, z_dec_nu = ', dec_tau_nu,z_dec_nu
             end if
           ! write(unit=25,fmt=56) tau, 1.d0/a-1.d0,a, dotmu_nu(i),1.0d0/(tau*dotmu_nu(i)),adot/a
!56          format(E14.8,1X,E14.8,1X,E14.8,1X,E14.8,1X,E14.8,1X,E14.8)

            if (tau < 0.001) then
                sdotmu_nu(i)=0
            else
                sdotmu_nu(i)=sdotmu_nu(i-1)+2._dl*dtau/(1._dl/dotmu_nu(i)+1._dl/dotmu_nu(i-1))
            end if
...
!!$
!!$     !Recompute visibility function as a check
        tau01=tauminn_nu
        adot0=adotrad
        a0=adotrad*tauminn_nu

        open(unit=26,file='visibility_neutrinos_test.dat',status='replace')
        do j1=1,nthermo_nu
            tau=tauminn_nu*exp((j1-1)*dlntau_nu)
            dtau=tau-tau01
            !Integrate Friedmann equation using inverse trapezoidal rule.
            a=a0+adot0*dtau
            adot=1/dtauda(a)
            ...
            if (sdotmu(j1) - sdotmu(nthermo)< -69) then
               ...
               emmu_nu(j1)=exp(sdotmu_nu(j1)-sdotmu_nu(nthermo_nu))
            end if
            vis_nu = emmu_nu(j1)*dotmu_nu(j1)
            write(unit=26,fmt=57) 1.d0/a-1.d0,a,tau,vis_nu,a*a*vis_nu/adot
            a0=a
            tau01=tau
            adot0=adot
        end do
57      format(E14.8,1X,E14.8,1X,E14.8,1X,E14.8,1X,E14.8)
        close(26)

        call splini(spline_data_nu,nthermo_nu)
        !call splder(cs2,dcs2,nthermo,spline_data)
        call splder(dotmu_nu,ddotmu_nu,nthermo_nu,spline_data_nu)
        call splder(ddotmu_nu,dddotmu_nu,nthermo_nu,spline_data_nu)
        call splder(dddotmu_nu,ddddotmu_nu,nthermo_nu,spline_data_nu)
        !call splder(emmu,demmu,nthermo,spline_data)

!        stop
    end subroutine inithermo_nu
    ...
    call inithermo_nu(taumin,taumax)  !FYCR
    ...
    if (CP%DerivedParameters) then
        ...
        ThermoDerivedParams( derived_zEQ ) = (grhob+grhoc)/(grhog+grhor_int+grhornomass+sum(grhormass(1:CP%Nu_mass_eigenstates))) -1
