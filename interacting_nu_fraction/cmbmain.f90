...
! perturbation equations, and InitialPower which provides the initial power spectrum.
  ...
  !        real(dl), dimension(:,:), pointer :: Delta_l_q
      ...
      real(dl),parameter :: qmin0=0.001_dl
...
!     Start when massive neutrinos are strongly relativistic.
            ...
            !Make sure the computation starts in the tightly-coupled regime
            if (taustart > tight_tau_nu ) then
                taustart = 0.1*tight_tau_nu
            end if

            if(taustart < taumin_check) taustart = taumin_check

            GetTauStart=taustart
      end function GetTauStart

        function GetTauStart_init(q)
        real(dl), intent(IN) :: q
        real(dl) taustart, GetTauStart_init
...
!     Start when massive neutrinos are strongly relativistic.
            ...
            GetTauStart_init=taustart
      end function GetTauStart_init
...
!     where taurst is the time when recombination starts - see inithermo
      ...
      taumin=GetTauStart_init(maxq)

      !Check if we are starting early enough for the interacting neutrinos
      T_nu_dec_approx = (1.0d6*eV/k_B)*(89.948433298516998384_dl/ &
                (1.221d22*CP%G_nu_eff**2*CP%Num_Nu_interacting))**(1.0d0/3.0d0)
      a_nu_dec_approx = CP%tcmb/T_nu_dec_approx

      print*, 'z_nu_dec = ' ,1.0d0/a_nu_dec_approx-1.0d0
      !print*, 'T_nu_dec = ' , T_nu_dec_approx/(10.0*(1.0d6*eV/k_B))
     ! stop
      tau_nu_dec = rombint(dtauda,0.0d0,a_nu_dec_approx,1.0d-5)
      print*, tau_nu_dec, taumin
      if (0.1*tau_nu_dec < taumin) taumin = 0.01d0*tau_nu_dec

      !Store value for checking when we solve for each k mode
      taumin_check = taumin
...
!     use a logarithmic spacing. closed case dealt with by SetClosedkValues
      ...
      real(dl) a,dgrho,zevol,dgq,dgs,sigma
      external dtauda
       ...
       if (fixq/=0._dl) then
        ...
        open(unit=35,file='Int_nu_evol_var_k_0.1_Geff_0.00001.txt')
        !call CreateTxtFile('evolve_q005.txt',103)
         do j=1,20000
          ...
          call derivs(EV,EV%ScalEqsToPropagate,tau,y,yprime)
           a=y(1)
           adotoa=sqrt((grhob/a+grhoc/a + grhog/a**2+grhor/a**2+grhor_int/a**2+grhov*a**2)/3)
          !ddelta= (yprime(3)*grhoc+yprime(4)*grhob)/(grhob+grhoc)
          !delta=(grhoc*y(3)+grhob*y(4))/(grhob+grhoc)
          !growth= ddelta/delta/adotoa
         
          dgrho=grhob/a*y(4)+grhoc/a*y(3) + grhog/a**2*y(EV%g_ix)+grhor/a**2*y(EV%r_ix)+grhor_int/a**2*y(EV%r_int_ix)
          dgq=grhob/a*y(5) + grhog/a**2*y(EV%g_ix+1)+grhor/a**2*y(EV%r_ix+1)+grhor_int/a**2*y(EV%r_int_ix+1)
          zevol=(0.5_dl*dgrho/EV%q + y(2))/adotoa
          sigma=(zevol+1.5_dl*dgq/EV%q2)
         
          if(EV%TightCoupling_nu .and. EV%TightCoupling) then
            dgs = grhog/a**2*EV%pig+grhor/a**2*y(EV%r_ix+2)+grhor_int/a**2*EV%pir_int
            write (35,'(13E18.9)') tau,y(1), sigma,dgs, y(2), y(3), y(4), y(EV%g_ix), &
                y(EV%g_ix+1), EV%pig,y(EV%r_int_ix), y(EV%r_int_ix+1),EV%pir_int
          elseif(.not.EV%TightCoupling_nu .and. EV%TightCoupling) then
             dgs = grhog/a**2*EV%pig+grhor/a**2*y(EV%r_ix+2)+grhor_int/a**2*y(EV%r_int_ix+2)
            write (35,'(13E18.9)') tau,y(1),sigma,dgs, y(2), y(3), y(4), y(EV%g_ix),&
                 y(EV%g_ix+1), EV%pig,y(EV%r_int_ix),y(EV%r_int_ix+1),y(EV%r_int_ix+2)

          elseif(EV%TightCoupling_nu .and. .not.EV%TightCoupling) then
             dgs = grhog/a**2*y(EV%g_ix+2)+grhor/a**2*y(EV%r_ix+2)+grhor_int/a**2*EV%pir_int
            write (35,'(13E18.9)') tau,y(1), sigma,dgs, y(2), y(3), y(4), y(EV%g_ix),&
                 y(EV%g_ix+1), y(EV%g_ix+2),y(EV%r_int_ix),y(EV%r_int_ix+1),EV%pir_int
          else
             dgs = grhog/a**2*y(EV%g_ix+2)+grhor/a**2*y(EV%r_ix+2)+grhor_int/a**2*y(EV%r_int_ix+2)
            write (35,'(13E18.9)') tau,y(1), sigma,dgs, y(2), y(3), y(4), y(EV%g_ix),&
                 y(EV%g_ix+1), y(EV%g_ix+2),y(EV%r_int_ix),y(EV%r_int_ix+1),y(EV%r_int_ix+2)
          end if
         end do
         close(35)
         stop
...
!     loop over wavenumbers.
     ...
     subroutine GetTransfer(EV,tau)
      ...
      ! print*,'finished init'
       ...
       !   print*,'finished Equations'
          if (global_error_flag/=0) return
          call outtransf(EV,y,MT%TransferData(:,EV%q_ix,i))
        !  print*,'finished outtransf'
       end do
