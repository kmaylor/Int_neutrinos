...
!  8*pi*G*rho*a**4.
        grhoa2=grhok*a2+(grhoc+grhob)*a+grhog+grhornomass+grhor_int
...
!Gauge-dependent perturbation equations
        ...
        type EvolutionVars
            ...
            integer r_int_ix !Index of the massless interacting neutrino hierarchy
    ...
    !True when using tight-coupling approximation (required for stability at early times)
            logical TightCoupling, TensTightCoupling,TightCoupling_nu
            real(dl) TightSwitchoffTime, TightSwitchoffTime_nu
    ...
    !Tensor vars
            ...
            real(dl) pig, pigdot,pir_int  !For tight coupling
            real(dl) poltruncfac
...
!precalculated arrays
       ...
       contains
        ...
        function next_nu_nq(nq) result (next_nq)
         ...
         integer ind, nu_i
         real(dl) cs2, opacity, dopacity,opacity_nu
         ...
         if (DoLateRadTruncation) then
          ...
          if (.not. EV%no_nu_multpoles) & !!.and. .not. EV%has_nu_relativistic .and. tau_switch_nu_massless ==noSwitch)  &
               ...
               tau_switch_no_nu_multpoles = max( tau_switch_no_nu_multpoles, 1.0*EV%TightSwitchoffTime_nu)
         ...
         next_switch = min(EV%TightSwitchoffTime_nu,tau_switch_ktau, tau_switch_nu_massless,&
         ...
         if (next_switch < tauend) then
             ...
             EVout=EV
             !print*, EV%no_nu_multpoles
             if (next_switch == EV%TightSwitchoffTime_nu) then
                EVout%TightCoupling_nu=.false.
                EVout%TightSwitchoffTime_nu = noSwitch
                ...
                y(EV%r_int_ix+2) = EV%pir_int
                !print*,  'y(EV%r_int_ix+2) = ', y(EV%r_int_ix+2) 
                call thermo_nu(tau,opacity_nu)
                y(EV%r_int_ix+3) = 3.0d0/7*y(EV%r_int_ix+2)*EV%k_buf/opacity_nu
          
            elseif (next_switch == EV%TightSwitchoffTime) then
              !TightCoupling           
        ...
        subroutine SetupScalarArrayIndices(EV, max_num_eqns)
          ...
          if (.not. EV%no_phot_multpoles) then
           ...
           if (.not. EV%no_nu_multpoles) then
            !Massless interacting neutrino multipoles
            EV%r_int_ix= neq+1
            if (EV%TightCoupling_nu) then
                neq=neq + 2
            else
                neq=neq + (EV%lmaxnr+1)
            end if
          ...
          maxeq = maxeq +  (EV%lmaxg+1)+2*(EV%lmaxnr+1)+EV%lmaxgpol-1
        ...
        subroutine CopyScalarVariableArray(y,yout, EV, EVout)
          ...
          if (.not. EV%no_nu_multpoles .and. .not. EVout%no_nu_multpoles) then
            if(EV%TightCoupling_nu .or. EVOut%TightCoupling_nu) then
                lmax=1
            else
                lmax = min(EV%lmaxnr,EVout%lmaxnr)
            end if
            yout(EVout%r_int_ix:EVout%r_int_ix+lmax)=y(EV%r_int_ix:EV%r_int_ix+lmax)
...
!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
        ...
        real(dl) clxr_int,qr_int,pir_int,qrdot_int,pirdot_int
...
!  8*pi*a*a*SUM[(rho_i+p_i)*v_i]
        ...
        grho=grhob_t+grhoc_t+grhor_t+grhog_t+grhov_t+grhor_int_t
        gpres=(grhog_t+grhor_t+grhor_int_t)/3+grhov_t*w_eff
        ...
        if (EV%no_nu_multpoles) then
            ...
            clxr=y(EV%r_ix)
            clxr_int=y(EV%r_int_ix)
            qr =y(EV%r_ix+1)
            qr_int =y(EV%r_int_ix+1)
            pir =y(EV%r_ix+2)
            pirdot =yprime(EV%r_ix+2)
            if (EV%TightCoupling_nu) then
                pir_int = EV%pir_int
                qrdot_int = yprime(EV%r_int_ix+1)
            else
                pir_int =y(EV%r_int_ix+2)
                pirdot_int=yprime(EV%r_int_ix+2)
            end if
        end if
       ...
       dgrho = dgrho + grhog_t*clxg+grhor_t*clxr + grhor_int_t*clxr_int
       dgq   = dgq   + grhog_t*qg+grhor_t*qr + grhor_int_t*qr_int
       dgpi  = dgpi  + grhor_t*pir + grhor_int_t*pir_int + grhog_t*pig
...
!  have to get z from eta for numerical stability
        ...
        if (EV%TightCoupling_nu) then
            pirdot_int = -dopac_nu(j)/opac_nu(j)*pir_int + (16._dl/27._dl)*k/opac_nu(j)*(-2.0_dl*adotoa*sigma  &
                 +etak/EV%Kf(1)-  dgpi/k + 3.0_dl*qrdot_int/4.0_dl)
        end if

        pidot_sum =  pidot_sum + grhog_t*pigdot + grhor_t*pirdot + grhor_int_t*pirdot_int
...
!  Initial conditions.
        ...
        integer, parameter :: i_clxg=1,i_clxr=2,i_clxc=3, i_clxb=4, &
                ...
                i_clxr_int=13,i_qr_int=14
        integer, parameter :: i_max = i_qr_int
        ...
        !as ensuring tight coupling is accurate enough
         ...
         !neutrino tight-coupling switch off
         EV%TightSwitchoffTime_nu = min(tight_tau_nu,Thermo_OpacityToTime_nu(EV%k_buf/ep))
...
!  k*tau, (k*tau)**2, (k*tau)**3
        ...
        om = (grhob+grhoc)/sqrt(3*(grhog+grhonu+grhor_int))
        omtau=om*tau
        !This is the density in free-streaming radiation over the total density in radiation
        Rv=grhonu/(grhonu+grhog+grhor_int)
        !print*, 'Rnu = ', Rv
        Rg = 1-Rv
...
!       !Tightly-coupled neutrinos initial conditions
        initv(1,i_clxr_int)= initv(1,i_clxg)
        initv(1,i_qr_int)=initv(1,i_qg)
...
!  Interacting Neutrinos
        y(EV%r_int_ix)=InitVec(i_clxr_int)
        y(EV%r_int_ix+1)=InitVec(i_qr_int)
...
!pi_r
 ...
 !write out clxc, clxb, clxg, clxn
        ...
        real(dl) clxc, clxb, clxg, clxr,clxr_int,k,k2
        ...
        Arr(Transfer_r_int) = clxr_int/k2
...
!  Internal variables.
        ...
        real(dl) dgq,grhob_t,grhor_t,grhor_int_t,grhoc_t,grhog_t,grhov_t,sigma,polter
        real(dl) qgdot,qrdot,pigdot,pirdot,vbdot,dgrho,adotoa
        real(dl) clxr_int,qr_int,pir_int,pirdot_int,qrdot_int,clxrdot_int
        real(dl) a,a2,z,clxc,clxb,vb,clxg,qg,pig,clxr,qr,pir
        real(dl) clxq, vq,  E2, dopacity,opacity_nu,dopacity_nu
...
!Get neutrino opacity
        if (EV%TightCoupling_nu) then
          call thermo_nu(tau,opacity_nu,dopacity_nu)
         else
          call thermo_nu(tau,opacity_nu)
        end if
        gpres=0
...
!       end if
       ...
       if (EV%no_nu_multpoles) then
        ...
        qr=-4._dl/3*z
        pir=0.0_dl
        clxr_int=-4.0_dl*dz/k
        qr_int=-4._dl/3*z
        pir_int=0.0_dl
       else
!  Massless neutrinos (now including interacting neutrinos)
        clxr=ay(EV%r_ix)
        qr  =ay(EV%r_ix+1)
        pir =ay(EV%r_ix+2)
        clxr_int=ay(EV%r_int_ix)
        qr_int  =ay(EV%r_int_ix+1)
        if (.not. EV%TightCoupling_nu) pir_int =ay(EV%r_int_ix+2)
       endif
       ...
       if (EV%no_phot_multpoles) then
         if (.not. EV%no_nu_multpoles) then
          z=(0.5_dl*dgrho/k + etak)/adotoa
          dz= -adotoa*z - 0.5_dl*dgrho/k
          clxg=-4.0_dl*dz/k-4/k*opacity*(vb+z)
          qg=-4._dl/3*z
         else
          clxg=clxr-4.0_dl/k*opacity*(vb+z)
          qg=qr
         end if
         pig=0
       else
...
!  Use explicit equation for vb if appropriate
         ...
         if (EV%TightCoupling) then
           ...
           !  ddota/a
            gpres=gpres+ (grhog_t+grhor_t+grhor_int_t)/3 +grhov_t*w_eff
            adotdota=(adotoa*adotoa-gpres)/2

            pig = 32._dl/45/opacity*k*(sigma+vb)

            if (EV%tightcoupling_nu) pir_int = (16.0/27.0)/opacity_nu*k*(sigma+3._dl*qr_int/4._dl)
    ...
    !  First-order approximation to baryon-photon splip
            ...
            !AL: First order slip seems to be fine here to 2e-4
             ...
             dgs = grhog_t*pig+grhor_t*pir+grhor_int_t*pir_int
...
!  Use explicit equations for photon moments if appropriate
            ...
            end if !.not. EV%tightcoupling
        end if !.not. EV%no_phot_multpoles

        if (EV%tightcoupling_nu) EV%pir_int = (16.0/27.0)/opacity_nu*k*(sigma+3._dl*qr_int/4._dl)
        ...
        if (.not. EV%no_nu_multpoles) then
            ...
            clxrdot_int=-k*(4._dl/3._dl*z+qr_int)
            ayprime(EV%r_int_ix)=clxrdot_int

            qrdot=EV%denlk(1)*clxr-EV%denlk2(1)*pir
            ayprime(EV%r_ix+1)=qrdot

            qrdot_int=EV%denlk(1)*clxr_int-EV%denlk2(1)*pir_int
            ayprime(EV%r_int_ix+1)=qrdot_int

            if (.not. EV%tightcoupling_nu) then
                if (.false.) then
                    !ufa approximation for k*tau>>1, more accurate when there are reflections from lmax
                    !Method from arXiv:1104.2933
                    pirdot_int= -3*pir_int*cothxor - clxrdot_int
                    ayprime(EV%r_int_ix+2)=pirdot_int
                    print*, 'error, this approx shouldnt be used here'
...
!                   ayprime(3+EV%lmaxg+7+1:EV%lmaxnr+EV%lmaxg+7)=0
                    ...
                    ix=EV%r_int_ix+2
                    if (EV%lmaxnr>2) then
                        pirdot_int=EV%denlk(2)*qr_int- EV%denlk2(2)*ay(ix+1)+8._dl/15._dl*k*sigma &
                            -(9.0_dl/10.0_dl)*opacity_nu*pir_int
                        ayprime(ix)=pirdot_int
                        do l=3,EV%lmaxnr-1
                            ix=ix+1
                            ayprime(ix)=(EV%denlk(l)*ay(ix-1) - EV%denlk2(l)*ay(ix+1))-ay(ix)*opacity_nu
                        end do
                        !Truncate the neutrino expansion
                        ix=ix+1
                        ayprime(ix)=k*ay(ix-1)- (EV%lmaxnr+1)*cothxor*ay(ix)-ay(ix)*opacity_nu
                    else
                        pirdot_int=EV%denlk(2)*qr_int +8._dl/15._dl*k*sigma &
                        -(9.0_dl/10.0_dl)*opacity_nu*pir_int
                        ayprime(ix)=pirdot_int
                    end if !EV%lmaxnr>2
                end if !EV%high_ktau_neutrino_approx
            end if !.not. EV%tightcoupling_nu
...
!  Evaluate the time derivatives of the tensor perturbations.
        ...
        real(dl), dimension(:),pointer :: neut,neutprime,E,B,Eprime,Bprime
        real(dl) q,aq,v
        real(dl)  grhob_t,grhor_t,grhoc_t,grhog_t,grhov_t,grhor_int_t,polter
...
! Also calculate gpres: 8*pi*p*a**2
        ...
        grho=grhob_t+grhoc_t+grhor_t+grhor_int_t+grhog_t+grhov_t
...
!Do massive neutrinos
        ...
        if (DoTensorNeutrinos) then
           ...
           end if  !Massive Neutrinos
        end if !Do tensor neutrinos
